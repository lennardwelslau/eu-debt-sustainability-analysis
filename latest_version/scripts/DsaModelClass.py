...
class DsaModel:
    ...
    def _import_ameco_data(self):
        ...
        self.pi_ea_t2 = self.df_ameco.loc[(self.df_ameco['year'] == self.start_year + 2) & (self.df_ameco['ISO'] == 'EA20'), 'gdp_def_pch'].values[0]
    ...
    def _import_interest_rate_data(self):
        ...
        self.df_t0_benchmark_rates = df_gov_rates.loc[df_gov_rates['year'] == self.start_year, ['ISO', '3M', '10Y']]
        self.df_t1_benchmark_rates = df_gov_rates.loc[df_gov_rates['year'] == self.start_year + 1, ['ISO', '3M', '10Y']]
        self.df_fwd_rates = df_gov_rates.loc[df_gov_rates['year'] == self.start_year + 1, ['ISO', '3M10Y', '10Y10Y']]
        ...
        self.benchmark_rate_st_t0 = self.df_t0_benchmark_rates.loc[(self.df_t0_benchmark_rates['ISO'] == self.country), '3M'].values[0]
        self.benchmark_rate_st_t1 = self.df_t1_benchmark_rates.loc[(self.df_t1_benchmark_rates['ISO'] == self.country), '3M'].values[0]
        self.benchmark_rate_lt_t0 = self.df_t0_benchmark_rates.loc[(self.df_t0_benchmark_rates['ISO'] == self.country), '10Y'].values[0]
        self.benchmark_rate_lt_t1 = self.df_t1_benchmark_rates.loc[(self.df_t1_benchmark_rates['ISO'] == self.country), '10Y'].values[0]

    def _import_inflation_data(self):
        """
        Import inflation expectations from Excel input data file.
        """
        self.df_infl_fwd_t10 = pd.read_excel('../data/InputData/deterministic_model_data.xlsx', sheet_name='inflation_fwd')
        self.infl_fwd_t10 = self.df_infl_fwd_t10.loc[(self.df_infl_fwd_t10['maturity'] == 10), 'infl_expectation'].values[0]
    ...
    def _clean_pi(self):
        ...
        if self.country in ['POL', 'ROU']:
            self.df_pi.loc[self.start_year + 10, 'gdp_def_pch'] = self.infl_fwd_t10 + (self.df_pi.loc[self.start_year + 2].values[0] - self.pi_ea_t2) / 2
            self.df_pi.loc[self.start_year + 30, 'gdp_def_pch'] = 2.5
            self.df_pi.loc[self.start_year + 10] = self.infl_fwd_t10 + (self.df_pi.loc[self.start_year + 2].values[0] - self.pi_ea_t2) / 2
            self.df_pi.loc[self.start_year + 30] = 2.5
        elif self.country in ['HUN']:
            self.df_pi.loc[self.start_year + 10, 'gdp_def_pch'] = self.infl_fwd_t10
            self.df_pi.loc[self.start_year + 30, 'gdp_def_pch'] = 3
            self.df_pi.loc[self.start_year + 10] = self.infl_fwd_t10
            self.df_pi.loc[self.start_year + 30] = 3
        else:
            self.df_pi.loc[self.start_year + 10, 'gdp_def_pch'] = self.infl_fwd_t10
            self.df_pi.loc[self.start_year + 30, 'gdp_def_pch'] = 2
            self.df_pi.loc[self.start_year + 10] = self.infl_fwd_t10
            self.df_pi.loc[self.start_year + 30] = 2
    ...
    def _clean_debt_redemption(self):
        ...
        self.share_lt_maturing[0] = self.share_lt_maturing_t0
        self.share_lt_maturing[(self.start_year + 11 - self.start_year):] = self.share_lt_maturing_t10
    ...
    def _project_market_rate(self):
        ...
        self.i_st[self.start_year + 11 - self.start_year] = self.fwd_rate_st
        self.i_lt[self.start_year + 11 - self.start_year] = self.fwd_rate_lt
        ...
        self.i_st[30:] = i_lt_t30 * yield_curve_coef
        self.i_lt[30:] = i_lt_t30

        # Set t+0 value
        self.i_st[0] = self.benchmark_rate_st_t0
        self.i_lt[0] = self.benchmark_rate_lt_t0

        # Set 2024 value
        self.i_st[1] = self.benchmark_rate_st_t1
        self.i_lt[1] = self.benchmark_rate_lt_t1
    ...
    def find_edp(self, spb_target=None):
        ...
        if self.inv_exception:
            self.edp_target[:np.min([self.inv_period, self.adjustment_period + 1])] -= 0.5
    ...
    def find_spb_deficit_resilience(self):
        ...
        if self.inv_exception:
            self.deficit_resilience_target[:np.min([self.inv_period, self.adjustment_period + 1])] -= 0.5
