...
def run_dsa(country_codes, results_dict, output_path, today, inv_shock=False):
    ...
    for counter, country in enumerate(country_codes):
        ...
        for adjustment_period in [4,7]:
            dsa = EcStochasticModel(country=country, adjustment_period=adjustment_period, inv_shock=inv_shock)
    ...
    with open(f'{output_path}/ec_dsa_results_dict_{today}.pkl', 'wb') as f:
...
def _save_spbs(results_dict, output_path, today):
    ...
    df_spb['binding_dsa'] = df_spb[['main_adjustment_debt_decline','adverse_r_g_debt_decline','lower_spb_debt_decline','financial_stress_debt_decline','stochastic_debt_decline']].max(axis=1)
    ...
    var_order = ['country',
                'adjustment_period', 
                'main_adjustment_debt_decline',
                'adverse_r_g_debt_decline',            
                'lower_spb_debt_decline', 
                'financial_stress_debt_decline',
                'stochastic_debt_decline', 
                'binding_dsa',
                'deficit_reduction', 
                'debt_safeguard',
                'deficit_resilience',
    ...
    df_spb.to_excel(f'{output_path}/ec_dsa_results_spb_{today}.xlsx', index=False)
...
def _save_ameco(results_dict, output_path, today):
    ...
    df_ameco.to_excel(f'{output_path}/ec_dsa_ameco_2024_{today}.xlsx', index=False)


################ Functions to plot charts ################

def plot_annex_charts(country_code_dict, results_dict, output_path, today):
    """
    ...
    _create_annex_charts(annex_chart_dict, output_path, country_code_dict, today)
...
def _calc_annex_charts(results_dict):
    ...
    for country, adjustment_period_dict in results_dict.items():
        ...
        for adjustment_period, scenario_dict in adjustment_period_dict.items():
            ...
            df_debt_chart = df[['y', 'spb_bca', 'pb', 'ob', 'ageing_component']].rename(columns={'y': 'year', 'spb_bca': 'Age-adjusted structural primary balance', 'pb': 'Primary balance', 'ob': 'Overall balance'})
            df_debt_chart['Structural primary balance'] = df_debt_chart['Age-adjusted structural primary balance'] + df_debt_chart['ageing_component'] 
            df_debt_chart.drop('ageing_component', axis=1, inplace=True)
            df_debt_chart.loc[:1+adjustment_period, 'Structural primary balance'] = df_debt_chart['Age-adjusted structural primary balance'].iloc[:2+adjustment_period]
...
def _create_annex_charts(annex_chart_dict, output_path, eu_code_dict, today):
    """
